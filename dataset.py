...
def setup_lra_clf_dataloaders(batch_size: int = 2, model: str = 'decoder'):
    t0 = time.time()
    train_ds = LRAClfDataset(Path('/content/drive/MyDrive/courses/ADL/aclImdb_2/aclImdb/train'))
    test_ds = LRAClfDataset(Path('/content/drive/MyDrive/courses/ADL/aclImdb_2/aclImdb/test'))
...
def setup_lra_ar_dataloaders(batch_size: int = 2, model: str = 'decoder'):
    t0 = time.time()
    train_ds = LRAARDataset(Path('/content/drive/MyDrive/courses/ADL/aclImdb_2/aclImdb/train'))
    test_ds = LRAARDataset(Path('/content/drive/MyDrive/courses/ADL/aclImdb_2/aclImdb/test'))
...
def setup_wikitext_dataloaders(batch_size: int = 2, model: str = 'decoder'):
    ...
    eval_dl = data.DataLoader(eval_ds, collate_fn=dc, batch_size=batch_size, shuffle=False, num_workers=4)
    test_dl = data.DataLoader(test_ds, collate_fn=dc, batch_size=batch_size, shuffle=False, num_workers=4)
    t1 = time.time()
    print(f'Dataloaders setup in {t1 - t0} seconds')
