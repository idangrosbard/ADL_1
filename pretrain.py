...
def do_epoch(model, dataloader, optimizer, loss, writer: SummaryWriter, device, train: bool = True):
    ...
    for batch in tqdm(dataloader, desc='train' if train else 'eval'):
        total_loss += do_batch(model, batch, optimizer, loss, device, train) / len(dataloader)
...
if __name__ == '__main__':
    ...
    model = EncoderLLM(train_dl.dataset.tokenizer.vocab_size, model_d, attn_d, ff_d, n_heads, n_layers)
