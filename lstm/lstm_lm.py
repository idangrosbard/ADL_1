...
class LSTM_LM(nn.Module):
    def __init__(self, corpus_size: int, d_model: int, d_hidden: int, n_layers: int):
        super(LSTM_LM, self).__init__()

        self.embed = nn.Embedding(corpus_size, d_model, corpus_size - 1)
        self.lstm_blocks = LSTMBlock(d_model, d_hidden, n_layers)
    ...
    def forward(self, x: Tensor, hs: Optional[Tensor], cs: Optional[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
        ...
        b_size = x.shape[0]
        if hs is None:
            hs = [h0.repeat(b_size) for h0 in self.h0s]
        if cs is None:
            cs = [c0.repeat(b_size) for c0 in self.c0s]
